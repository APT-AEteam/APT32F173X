...
csi_error_t csi_lin_init(csp_lin_t *ptLinBase, csi_lin_config_t *ptLinCfg)
{
	...
	if(ptLinCfg->eClkSrc > LIN_CLKSRC_DIV8)
		return CSI_ERROR;
	else
		csp_usart_set_ckdiv(ptLinBase, ptLinCfg->eClkSrc);						//clk source
	...
	//lin
	csp_usart_lin_set_ver(ptLinBase,(lin_ver_e)ptLinCfg->eLinVer);						//lin1.2/lin2.0
	csp_usart_lin_format(ptLinBase, (lin_chk_e)ptLinCfg->eCheck, ptLinCfg->hwWkUpTime);	//checksum selection and wakeup time
...
{
	csp_usart_clr_isr(ptLinBase, (usart_isr_e)eIntSrc);

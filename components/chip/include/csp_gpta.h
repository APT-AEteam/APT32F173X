...
#define GPTA_SWSYNEN_POS	     (2)
#define GPTA_SWSYNEN_MSK         (0x1ul << GPTA_SWSYNEN_POS)
typedef enum
{
	GPTA_SW_START = 0,
	GPTA_SYNC_START
...
static inline void csp_gpta_set_mode(csp_gpta_t *ptGptaBase, gpta_md_e eMode)
{
...
static inline void csp_gpta_set_capmd(csp_gpta_t *ptGptaBase, gpta_capmd_e eMode)
{
...
static inline void csp_gpta_burst_enable(csp_gpta_t *ptGptaBase,gpta_cgsrc_e eCgsrc)
{
	ptGptaBase -> CR |= GPTA_BURST_MSK;
}

static inline void csp_gpta_burst_disable(csp_gpta_t *ptGptaBase,gpta_cgsrc_e eCgsrc)
{
	ptGptaBase -> CR &= ~GPTA_BURST_MSK;
}
...
static inline void csp_gpta_prdld_mod(csp_gpta_t *ptGptaBase, gpta_prdld_e ePrdrld)
{
	ptGptaBase -> CR = (ptGptaBase->CR & ~(GPTA_PRDLD_MSK)) | (ePrdrld << GPTA_PRDLD_POS);
}

static inline void csp_gpta_set_start_src(csp_gpta_t *ptGptaBase, gpta_swsynen_e eVal)
{
	ptGptaBase -> CR = (ptGptaBase->CR & (~GPTA_SWSYNEN_MSK))| (eVal <<GPTA_SWSYNEN_POS);
}
...
static inline void csp_gpta_set_opmd(csp_gpta_t *ptGptaBase, gpta_opmd_e eVal)
{
...
//syncr 已經確認
static inline void csp_gpta_sync_enable(csp_gpta_t *ptGptaBase, uint8_t byCh)
{  
	ptGptaBase -> REGPROT = GPTA_REGPROT;
	ptGptaBase -> SYNCR |= GPTA_SYNC_MSK(byCh);
}

static inline void csp_gpta_sync_disable(csp_gpta_t *ptGptaBase, uint8_t byCh)
{   
	ptGptaBase -> REGPROT = GPTA_REGPROT;
	ptGptaBase -> SYNCR &= ~GPTA_SYNC_MSK(byCh);
}

static inline void csp_gpta_set_sync_mode(csp_gpta_t *ptGptaBase, uint8_t byCh, gpta_syncmd_e eMode)
{   
	ptGptaBase -> REGPROT = GPTA_REGPROT;
...
{   
	ptGptaBase -> REGPROT = GPTA_REGPROT;
	ptGptaBase -> SYNCR = (uint32_t)((ptGptaBase -> SYNCR & ~GPTA_TRGO0SEL_MSK) | (byTrgin << GPTA_TRGO0SEL_POS));
}

static inline void csp_gpta_sync_trg1sel(csp_gpta_t *ptGptaBase, uint8_t byTrgin)
{   
	ptGptaBase -> REGPROT = GPTA_REGPROT;;
	ptGptaBase -> SYNCR = (uint32_t)((ptGptaBase -> SYNCR & ~GPTA_TRGO1SEL_MSK) | (byTrgin << GPTA_TRGO1SEL_POS));
}

static inline void csp_gpta_sync_rearm(csp_gpta_t *ptGptaBase, uint8_t byCh)
{   
	ptGptaBase -> REGPROT = GPTA_REGPROT;;
	ptGptaBase -> SYNCR = ptGptaBase -> SYNCR | GPTA_REARM_MSK(byCh);
}
...
//EVTRG  已经确认
static inline void csp_gpta_trg_cntxiniten_enable(csp_gpta_t *ptGptaBase, uint8_t byCh)///
{
	ptGptaBase -> EVTRG |= GPTA_INITEN_MSK_CNT(byCh);
}

static inline void csp_gpta_trg_cntxiniten_disable(csp_gpta_t *ptGptaBase, uint8_t byCh)///
{
	ptGptaBase -> EVTRG &= ~GPTA_INITEN_MSK_CNT(byCh);
}

static inline void csp_gpta_evtrg_enable(csp_gpta_t *ptGptaBase, uint8_t byCh)
{
	ptGptaBase -> EVTRG |= GPTA_OUTEN_MSK_TRG(byCh);
}

static inline void csp_gpta_evtrg_disable(csp_gpta_t *ptGptaBase, uint8_t byCh)
{
	ptGptaBase -> EVTRG &= ~GPTA_OUTEN_MSK_TRG(byCh);
}

static inline void csp_gpta_set_evtrg(csp_gpta_t *ptGptaBase, uint8_t byCh, gpta_trgsrc_e eSrc)
{

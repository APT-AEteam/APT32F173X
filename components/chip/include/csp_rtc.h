...
///CR
#define RTC_INIT            (0x1ul)
#define RTC_BSY				(0x2ul)
#define RTC_ALMAE           (0x1ul << 3)
#define RTC_ALMBE           (0x1ul << 4)
...
static inline void csp_rtc_set_src_clk(csp_rtc_t *ptRtcBase,rtc_clksrc_e eClkSrc,uint8_t byDiva, uint16_t hwDivs)						
{ 
	ptRtcBase->CCR = (ptRtcBase->CCR & (~RTC_CLKSRC_MSK) & (~RTC_DIVA_MSK)& (~RTC_DIVS_MSK)) | (eClkSrc << RTC_CLKSRC_POS)|(byDiva << RTC_DIVA_POS)| (hwDivs << RTC_DIVS_POS) | (RTC_CLKEN);
}
...
static inline void csp_rtc_set_key(csp_rtc_t *ptRtcBase)							
{ 
	ptRtcBase->KEY = 0xCA53;
}

static inline void csp_rtc_clr_key(csp_rtc_t *ptRtcBase)							
{ 
	ptRtcBase->KEY = 0x0;
}

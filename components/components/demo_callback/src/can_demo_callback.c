...
int can_send_int_callback_demo(void)
{
	...
	//CAN发送报文配置，每个报文配置在一个报文通道中(CAN_CH1~CAN_CH32)
	for(i = CAN_CH1; i < CAN_CH3; i++)						
	{
		...
		csi_can_set_msg_tx(CAN0, i, &tCanTxConfig);			//配置发送报文
	}
	...
	csi_can_msg_send_int(CAN0, CAN_CH1, 8);					//发送报文通道1中的报文
	udelay(300);											//多个报文发送时，报文之间需要加几百微秒延时
	csi_can_msg_send_int(CAN0, CAN_CH2, 8);					//发送报文通道2中的报文
...
//		tCanRxConfig.tMsk.wExtIdMsk	 	= 0x00000;			//扩展模式ID mask, 标准模式下忽略
	...
	csi_can_msg_receive_int(CAN0, s_tCanRecv, CAN_CH3, 3);	//接收报文数据到s_tCanRecv数组，开启所有状态中断和对应报文接收通道中断
	csi_can_start(CAN0);									//使能(打开)CAN模块

...
int cmp_dfcr_test(void)
{
	...
	tCmpCfg.byNhystpol = CMP_PHYST_POL_0mv;	          //比较器输入迟滞特性极性选择
	tCmpCfg.byPolarity = CMP_POL_OUT_UNDIRECT;           //比较器输出极性选择 0:不反向
	tCmpCfg.byCpoSel = CMP_CPOS_OUT_IN;	              //CMP_OUT管脚上输出信号选择 0h：滤波前信号直接输出 	1h：滤波后信号输出 
	tCmpCfg.wInt = CMP_INTSRC_NONE;	      		      //中断模式
	csi_cmp_init(CMP0,&tCmpCfg);
	
	//csp_cmp_speedup(CMP0 , SPEEDUP_FAST);
	//csp_cmp_doublei(CMP0 , DOUBLEI_DOUBLE);
	...
	csi_cmp_dflt1_config(CMP0,DISABLE,&tCmpDflt1Cfg);
...
int cmp_wfcr_test(void)
{
	...
	tCmpWfcrCfg.byClkDiv  = 10;                   //时钟分频
	tCmpWfcrCfg.byDcnt    = 0;                   //窗口延迟
	tCmpWfcrCfg.hwWcnt    = 0xf00;                 //窗口计数
	...
	csi_bt_timer_init(BT0, 8000);		//初始化BT0, 定时2000us； BT定时，默认采用PEND中断

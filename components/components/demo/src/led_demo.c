...
{	
	...
	while(1)
	{
		switch(byDisplayStatus)
		{
			...
			case 1:
				csi_led_lighton(LED);
				csi_led_set_blink_control(LED, LED_BLK_OFF,(0x01&LED_BLK_MSK));//disable COM0
				for(uint8_t i = 0; i < 4; i++)
				{
					csi_led_write_data(LED, i, g_byLedData[i]);
					mdelay(50);
			...
			case 2:
				csi_led_lighton(LED);
				csi_led_set_blink_control(LED, LED_BLK_OFF,(0x02&LED_BLK_MSK));//disable COM1
				for(uint8_t i = 0; i < 4; i++)
				{
					csi_led_write_data(LED, i, g_byLedData[i]);
					mdelay(50);
			...
			case 3:
				csi_led_lighton(LED);
				csi_led_set_blink_control(LED, LED_BLK_OFF,(0x04&LED_BLK_MSK));//disable COM2
				for(uint8_t i = 0; i < 4; i++)
				{
					csi_led_write_data(LED, i, g_byLedData[i]);
					mdelay(50);
				...
				byDisplayStatus = 0;
				mdelay(1);
				csi_led_set_blink_control(LED, LED_BLK_ON,(0x07&LED_BLK_MSK));//enable COM0/1/2
			break;

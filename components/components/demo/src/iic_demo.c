...
#if (USE_I2C_CALLBACK == 0)

/** \brief	i2c_int_handler: I2C中断服务函数
 * 
 *  \brief 	I2C发生中断时会调用此函数，函数在interrupt.c里定义为弱(weak)属性，默认不做处理；用户用到I2C中
...
void iic_master_eeprom_demo(void)
{
	...
	csi_iic_int_enable(I2C0, IIC_INTSRC_NONE);              //使能需要的中断
...
void iic_slave_demo(void)
{
	...
	csi_iic_int_enable(I2C0, IIC_INTSRC_SCL_SLOW | IIC_INTSRC_STOP_DET | IIC_INTSRC_RD_REQ | IIC_INTSRC_RX_FULL | IIC_INTSRC_TX_ABRT|IIC_INTSRC_TX_OVER);   //使能需要的中断
...
//	tDmaConfig.wInt  = DMA_INTSRC_NONE;   		//使用TCIT中断
	...
	csi_iic_master_init(I2C0,&g_tIicMasterCfg);				//主机初始化
	csi_iic_int_enable(I2C0, IIC_INTSRC_NONE);     //使能需要的中断
	csi_i2c_set_rxdma(I2C0, I2C_RDMA_EN, I2C_RDMA_FIFO_NSPACE);
	csi_i2c_set_txdma(I2C0, I2C_TDMA_EN, I2C_TDMA_FIF0_TX_FLSEL); 
	
	g_wTxBuff[8] |=  I2C_CMD_STOP;     // 最后一个数添加stop
	
	csi_iic_disable(I2C0);
	csi_iic_set_taddr(I2C0,0xa0 >> 1);

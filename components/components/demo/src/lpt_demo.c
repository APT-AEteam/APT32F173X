...
int lpt_timer_demo(void)
{
	...
	csi_lpt_timer_init(LPT,LPT_CLK_PCLK_DIV4,200);       //初始化lpt,选用内部超低功耗时钟,定时200ms,默认采用PEND中断
...
int lpt_pwm_demo(void)
{
	...
	csi_pin_set_mux(PA4, PA4_LPT_OUT);	                           //将PA4设为LPT_OUT
//	csi_pin_set_mux(PC1, PC1_LPT_OUT);	                           //将PC1设为LPT_OUT
